...
const Sidebar = ({ title, subtitle, children, className = '' }) => {
  ...
  return (
    <aside className={`w-full lg:w-64 shrink-0 flex flex-col ${className}`}>
      {/* * Mobile Header Trigger Bar */}
      <div className="lg:hidden flex items-center justify-between p-4 bg-white dark:bg-[#0b1329] border border-gray-200 dark:border-gray-800 rounded-xl mb-3 shadow-sm">
        <div>
      ...
      <div
        className={`${
          isOpen ? 'block' : 'hidden'
        } lg:flex flex-col flex-1 h-full bg-white dark:bg-[#0b1329] border border-gray-200 dark:border-gray-800 rounded-2xl p-4 shadow-sm space-y-4`}
      >
        {/* * Desktop Header Block */}
        {(title || subtitle) && (
          <div className="hidden lg:block pb-3 px-2 border-b border-gray-100 dark:border-gray-800">
            {title && (
